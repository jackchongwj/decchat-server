    ...
    public class FriendsServices : IFriendService
    {
        ...
        public async Task<IEnumerable<Users>> AddFriends(Friends friends)
        {
            ...
            if (!result.IsNullOrEmpty())
            {
                try
                {
                    await _hubContext.Clients.Group("User"+ friends.ReceiverId).SendAsync("UpdateSearchResults", friends.SenderId);
                    await _hubContext.Clients.Group("User" + friends.SenderId).SendAsync("UpdateSearchResults", friends.ReceiverId);
                    await _hubContext.Clients.Group("User" + friends.ReceiverId).SendAsync("UpdateFriendRequest", result);
                }
                catch (Exception ex)
                {
        ...
        {
            return await _repo.CheckFriendExist(friends);
        }
    }
}
